...
export class INIT1707671457671 implements MigrationInterface {
    name = 'INIT1707671457671'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`orders\` (\`id\` bigint NOT NULL, \`status\` int NOT NULL DEFAULT '0', \`total_price\` int NOT NULL, \`date_start\` datetime NOT NULL, \`date_end\` datetime NOT NULL, \`paymentMethodId\` int NULL, \`tableId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`orders\` ADD CONSTRAINT \`FK_c137e0afcc291b9135d3e0c3d4e\` FOREIGN KEY (\`paymentMethodId\`) REFERENCES \`payment_method\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
